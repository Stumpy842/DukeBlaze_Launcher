...
namespace DragDukeLauncher
{
    public partial class MainWindow : Form
    {
        ...
        private List<TreeNode> ColoredNodes = [];
        ...
        {
            using OpenFileDialog cfgBrowser = new OpenFileDialog();
        ...
        private void SaveAsPreset()
        {
            void AddAsNew()
            {
                string pName = PresetNameTextBox.Text;
        ...
        private string GetNC(bool Folders = false)
        {
            ...
            if (Folders)
            {
                switch (PreventFolderCollisions)
                {
                    case Settings.PC.Allow: nc = nsw.rbFolderAll.Text; break;
                    case Settings.PC.Folder: nc = nsw.rbFolderParent.Text; break;
                    case Settings.PC.Global: nc = nsw.rbFolderGlobal.Text; break;
                    default: throw new ArgumentException("Unknown parameter", PreventFolderCollisions.ToString());
                }
            }
            else
            {
                switch (PreventPresetCollisions)
                {
                    case Settings.PC.Allow: nc = nsw.rbPresetAll.Text; break;
                    case Settings.PC.Folder: nc = nsw.rbPresetFolder.Text; break;
                    case Settings.PC.Global: nc = nsw.rbPresetGlobal.Text; break;
                    default: throw new ArgumentException("Unknown parameter", PreventPresetCollisions.ToString());
                }
            }
            return nc;
        }
        ...
        internal void SelectItem()  // This method is looking like Spaghetti code now, sorry ;-)
        {
            ...
            //var sNode = PresetTree.SelectedNode;
            if (PresetTree.SelectedNode is null) return;
            if (PresetTree.SelectedNode.Parent is null)
            {
                EditFolderButton.Enabled = false;
        ...
        private void PresetTree_DragDrop(object sender, DragEventArgs e)
        {
            ...
            if (draggedNode.Parent is null) return;
            ...
            try
            {
                if (!draggedNode.Equals(targetNode) && !ContainsNode(draggedNode, targetNode))
                {
                    ...
                    {
                        if (!targetNode.Text.StartsWith(Tools.FolderIcon) && !draggedNode.Text.StartsWith(Tools.FolderIcon))
                        {
                            RecoverLastSelectedNode();
                            return;
                        }
        ...
        {
            if (node2.Parent is null) return false;
        ...
        {
            using OpenFileDialog exeBrowser = new OpenFileDialog();
        ...
        private void RemoveAllFoldersOrPresets()
        {
            ...
            TaskDialogButtonCollection cButtons = new()
            {
        ...
        private string CopyDatFiles(string Src, string Dst)
        {
            try
            {
                    ...
                    try
                    {
                        ...
                        if (fi.Length > 0)
                        {
                            ...
                            if (optConfirmOverWrite &&
                                ...
                                new TaskDialogButtonCollection { TaskDialogButton.OK, TaskDialogButton.Cancel }, 1,
