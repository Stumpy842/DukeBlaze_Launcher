...
namespace DukeBlazeLauncher
{
    ...
    public partial class ImportMapsWindow : Form
    {
        ...
        private NaturalStringComparer comparer = new();
            ...
            if (fileBrowser.ShowDialog(this) == DialogResult.OK)
            {
                if (fileBrowser.FileNames.Length > 0)
                {
                    foreach (string file in fileBrowser.FileNames)
                        // Ext to lowercase before matching
                        // Steve - 04/27/2025 19:22:13
                        if (Path.GetExtension(file).ToLowerInvariant() == mapExt)
        ...
        {
            // Sort Files array by natural sort
            // Steve - 04/28/2025 21:59:27
            Files.Sort(comparer);
        ...
        private void ImportMapsWindow_FormClosing(object sender, FormClosingEventArgs e)
        {
            SaveSettings();
        }
    }
